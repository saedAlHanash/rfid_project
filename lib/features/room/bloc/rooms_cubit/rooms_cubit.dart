...
import 'package:rfid_project/features/room/data/request/create_room_request.dart';
...
class RoomsCubit extends MCubit<RoomsInitial> {
  ...
  void getDataFromCache() => getFromCache(fromJson: Room.fromJson, state: state);
  ...
  Future<Pair<List<Room>?, String?>> _getData() async {
    final response = await APIService().callApi(
      type: ApiType.get,
      url: PostUrl.rooms,
      query: {'division_id': state.id.toString()},
    );

    if (response.statusCode.success) {
      return Pair(Rooms.fromJson(response.jsonBody).data, null);
    } else {
  ...
  Future<void> deleteNow({required String id}) async {
    ...
    if (response.statusCode.success) {
      await deleteRoomFromCache(item.id.toString());
    } else {
